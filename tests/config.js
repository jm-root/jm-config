...
(function(){
    ...
    var listener = function(doc){
        logger.debug('onMessage: %s', utils.formatJSON(doc));
    };

    config.on('setConfig', listener);
    config.on('setConfigs', listener);
    config.on('delConfig', listener);
    config.on('delRoot', listener);
